...
bool cariBuku(BukuNode* head, string judulDicari) {
    ...
    if (head == nullptr) {
        return false; // Ketika list kosong maka nilai yang dikembalikan adalah false
    }

    BukuNode* temp = head; // Memulai penulusuran dari head menggunakan pointer temp
    do { // Looping penelusuran
        if (temp->judulBuku == judulDicari) {
            return true; // Ketika judul buku yang dicari ditemukan maka nilai yang dikembalikan adalah true
        }
